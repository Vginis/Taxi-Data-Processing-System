...
countsDF=spark.read.option("header","true").option("inferSchema","true").csv("/mnt/taxidata/output4.csv")
...
quadrants_df.coalesce(1).write.csv(
    "wasbs://"+storage_container_name+"@"+storage_account_name+".blob.core.windows.net/address-temp4",
    header=True,
)
file_path = [
    file.path
...
dbutils.fs.cp(
    file_path,
    "wasbs://"+storage_container_name+"@"+storage_account_name+".blob.core.windows.net/output4.csv",
)
dbutils.fs.rm(
    "wasbs://"+storage_container_name+"@"+storage_account_name+".blob.core.windows.net/address-temp4",
    recurse=True,
