...
for row in dataRows:
    if (
        float(row[1]) >= 10
        and haversine(longtitude, latitude, float(row[3]), float(row[4])) >= 4.8
        and haversine(longtitude, latitude, float(row[3]), float(row[4])) <= 5.2
    ):
        routes.append(row)
...
dbutils.fs.cp(
    ...
    + "@"
    + storage_account_name
    + ".blob.core.windows.net/output5.csv",
)
dbutils.fs.rm(
    "wasbs://"
    + storage_container_name
